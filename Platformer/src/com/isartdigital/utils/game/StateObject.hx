...
class StateObject<T:DisplayObjectContainer> extends StateMachine
{
	 ...
	 */
	public var isAnimEnded (get_isAnimEnded, null):Bool;
	...
	private function createCollider ():Void {
		...
		for (lCollider in lColliders.keys()) {
			...
			if (Std.is(lColliders[lCollider], Rectangle)) {
				lChild.graphics.drawRect(lColliders[lCollider].x, lColliders[lCollider].y, lColliders[lCollider].width, lColliders[lCollider].height);
			}
			else if (Std.is(lColliders[lCollider], Circle)) {
				lChild.graphics.drawCircle(lColliders[lCollider].x,lColliders[lCollider].y,lColliders[lCollider].radius);
			}
			...
			if (Std.is(lColliders[lCollider], Point)) {
				
				lChild.x = lColliders[lCollider].x;
				lChild.y = lColliders[lCollider].y;
			}
