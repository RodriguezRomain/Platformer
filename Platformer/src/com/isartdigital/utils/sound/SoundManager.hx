...
class SoundManager 
{
	...
	**/
	public static function setupSoundsData(pJson:Dynamic, pGameloopDispatcher:EventDispatcher) : Array<String> {
		...
		for (sound in Reflect.fields(lFxs)) {
			...
			lPath = "assets/sounds/" + lSoundData.asset + ".mp3";
			lPaths.push(lPath);
			addSound(sound, new SoundFX(lPath, lSoundData.volume * lVolumes.fxs, pGameloopDispatcher));
		}
		...
		for (sound in Reflect.fields(lMusic)) {
			...
			lPath = "assets/sounds/" + lSoundData.asset + ".mp3";
			lPaths.push(lPath);
			addSound(sound, new SoundFX(lPath, lSoundData.volume * lVolumes.musics, pGameloopDispatcher));
		}
