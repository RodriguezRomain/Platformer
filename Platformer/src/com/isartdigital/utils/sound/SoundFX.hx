...
class SoundFX extends EventDispatcher
{
	...
	/**
	  * Emitter de gameloop
	**/
	private var gameLoopEmitter:EventDispatcher;
	...
	**/
	...
	public function new(pName:String, pVolume:Float, pGameLoopEmitter:EventDispatcher) 
	{
		soundPath 			 = pName;
		initialVolume   = volume = pVolume;
		gameLoopEmitter  = pGameLoopEmitter;
		super();
	}
	...
	public function stop ():Void {
		...
		gameLoopEmitter.removeEventListener(EventType.GAME_LOOP, doFadeIn);
		gameLoopEmitter.removeEventListener(EventType.GAME_LOOP, doFadeOut);
		channel.stop();
	...
	public function fadeIn (pFadeSpeed:Float = FADE_SPEED, pTargetedVolume:Float = -1, pStartTime:Int = 0, pLoops:Int = 0): Void {
		...
		gameLoopEmitter.addEventListener(EventType.GAME_LOOP, doFadeIn);
	}
	...
	public function fadeOut (pFadeSpeed:Float = FADE_SPEED):Void {
		...
		gameLoopEmitter.removeEventListener(EventType.GAME_LOOP, doFadeIn);
		gameLoopEmitter.addEventListener(EventType.GAME_LOOP, doFadeOut);
	}
